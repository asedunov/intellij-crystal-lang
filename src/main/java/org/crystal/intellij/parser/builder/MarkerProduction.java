...
final class MarkerProduction extends IntArrayList {
  ...
  void addBefore(LazyPsiBuilder.ProductionMarker marker, LazyPsiBuilder.ProductionMarker anchor) {
    add(indexOf(anchor), marker.markerId);
  }

  private int indexOf(LazyPsiBuilder.ProductionMarker marker) {
  ...
  void addMarker(LazyPsiBuilder.ProductionMarker marker) {
    add(marker.markerId);
  }

  void rollbackTo(LazyPsiBuilder.ProductionMarker marker) {
  ...
  boolean hasErrorsAfter(@NotNull LazyPsiBuilder.StartMarker marker) {
    for (int i = indexOf(marker) + 1; i < size(); ++i) {
      LazyPsiBuilder.ProductionMarker m = getStartMarkerAt(i);
      if (m != null && hasError(m)) return true;
    }
    return false;
  }

  private boolean hasError(LazyPsiBuilder.ProductionMarker marker) {
    return marker instanceof LazyPsiBuilder.ErrorItem || myOptionalData.getDoneError(marker.markerId) != null;
  }

  void dropMarker(@NotNull LazyPsiBuilder.StartMarker marker) {
    if (marker.isDone()) {
  ...
  void addDone(LazyPsiBuilder.StartMarker marker, @Nullable LazyPsiBuilder.ProductionMarker anchorBefore) {
    add(anchorBefore == null ? size() : indexOf(anchorBefore), -marker.markerId);
  }

  @Nullable
  LazyPsiBuilder.ProductionMarker getMarkerAt(int index) {
    int id = getInt(index);
    return myPool.get(id > 0 ? id : -id);
  }

  @Nullable
  LazyPsiBuilder.ProductionMarker getStartMarkerAt(int index) {
    int id = getInt(index);
    return id > 0 ? myPool.get(id) : null;
  }

  @Nullable
  LazyPsiBuilder.StartMarker getDoneMarkerAt(int index) {
    int id = getInt(index);
    return id < 0 ? (LazyPsiBuilder.StartMarker)myPool.get(-id) : null;
  }
  ...
  void confineMarkersToMaxLexeme(int markersBefore, int lexemeIndex) {
    for (int k = markersBefore - 1; k > 1; k--) {
      int id = a[k];
      LazyPsiBuilder.ProductionMarker marker = myPool.get(Math.abs(id));
      boolean done = id < 0;
  ...
  void doHeavyChecksOnMarkerDone(@NotNull LazyPsiBuilder.StartMarker doneMarker, @Nullable LazyPsiBuilder.StartMarker anchorBefore) {
    ...
    for (int i = endIdx - 1; i > idx; i--) {
      LazyPsiBuilder.ProductionMarker item = getStartMarkerAt(i);
      if (item instanceof LazyPsiBuilder.StartMarker) {
        LazyPsiBuilder.StartMarker otherMarker = (LazyPsiBuilder.StartMarker)item;
  ...
  void assertNoDoneMarkerAround(@NotNull LazyPsiBuilder.StartMarker pivot) {
    int pivotIndex = indexOf(pivot);
    for (int i = pivotIndex + 1; i < size(); i++) {
      LazyPsiBuilder.StartMarker m = getDoneMarkerAt(i);
