...
OCT_DIGIT = [0-7]
BIN_DIGIT = [01]
HEX_INT_BODY = 0x{HEX_DIGIT}+(_ {HEX_DIGIT}+)*
OCT_INT_BODY = 0o{OCT_DIGIT}+(_ {OCT_DIGIT}+)*
BIN_INT_BODY = 0b{BIN_DIGIT}+(_ {BIN_DIGIT}+)*
DEC_INT_BODY = {DEC_DIGIT}+(_ {DEC_DIGIT}+)*
INT_SUFFIX = (i | u) ("8" | "16" | "32" | "64" | "128")
INTEGER_LITERAL = ({HEX_INT_BODY} | {OCT_INT_BODY} | {BIN_INT_BODY} | {DEC_INT_BODY})(_? {INT_SUFFIX})?
FLOAT_SUFFIX = f ("32" | "64")
EXP_SUFFIX = [eE] [+-]? {DEC_INT_BODY}
FLOAT_LITERAL = ({HEX_INT_BODY} | {OCT_INT_BODY} | {BIN_INT_BODY} | {DEC_INT_BODY} (\. {DEC_INT_BODY})? {EXP_SUFFIX}?) (_? {FLOAT_SUFFIX})?
...
<MACRO_CONSUME_ESCAPED_CONTROL_PREFIX> {
  ...
  "\\{%" {ASCII_WHITESPACE}* ("for" | "if" | "unless") {
